...
// Initialization commands for the display controller (same as original)
...
// Core 0: Main Game Logic and Display Task - Same as working original but async
#[embassy_executor::task]
async fn game_logic_task(
    mut rng: Rng,
    mut dpi: Dpi<'static, Blocking>,
    mut dma_tx: DmaTxBuf,
    mut frame_buf: FrameBuf<Rgb565, HeapBuffer<Rgb565, BUFFER_SIZE>>,
) {
    info!("[CORE 0] Game logic task started - running Embassy on main core");
    ...
    loop {
        ...
        // Reset after certain generations
...
async fn main(spawner: Spawner) -> ! {
    ...
    info!("Display initialized, starting Embassy task");
    ...
    let frame_buf = FrameBuf::new(heap_buffer, LCD_H_RES.into(), LCD_V_RES.into());
