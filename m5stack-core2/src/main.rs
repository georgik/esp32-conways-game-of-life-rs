...
fn render_system(
    ...
    let line1 = "Rust no_std M5Stack-Core2";
...
fn main() -> ! {
    ...
    // --- Initialize I2C for Core2 (GPIO21=SDA, GPIO22=SCL) ---
    ...
    // Create a RefCell for I2C sharing and leak it to get a 'static reference
    let i2c_ref_cell = alloc::sync::Arc::new(core::cell::RefCell::new(i2c));
    let axp_i2c = embedded_hal_bus::i2c::RefCellDevice::new(&i2c_ref_cell);
    ...
    let reset = Output::new(peripherals.GPIO4, Level::High, OutputConfig::default());
