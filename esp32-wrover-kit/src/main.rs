...
fn main() -> ! {
    ...
    let cs_output = Output::new(peripherals.GPIO22, Level::High, OutputConfig::default());
    let spi_delay = Delay::new();
    let spi_device = ExclusiveDevice::new(spi, cs_output, spi_delay).unwrap();

    // LCD interface
    let lcd_dc = Output::new(peripherals.GPIO21, Level::Low, OutputConfig::default());
